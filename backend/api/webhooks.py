...
def _ensure_ad_names_table(db_path: str) -> None:
    """Create ad_names table if it doesn't exist yet."""
...
def _learn_ad_hierarchy(
    ...
    *,
    ...
    """Auto-learn campaign → adset → ad hierarchy from UTM tracking params.

    Called on every pixel hit that has a detected platform.  Stores the
    relationship into ``ad_names`` so the report can build the drill-down
    tree without any ad-platform API keys.

    UTM mapping reminder (what visitors arrive with):
        ...
        if campaign_id:
            conn.execute(
                """INSERT OR REPLACE INTO ad_names
                   ...
                   VALUES (?, 'campaign', ?, ?, '', 'pixel', ?)""",
                (platform, campaign_id, campaign_id, now),
            )

        # Ad set (parent = campaign)
        if adset_id:
            conn.execute(
                """INSERT OR REPLACE INTO ad_names
                   (platform, entity_type, entity_id, name, parent_id, source, updated_at)
                   VALUES (?, 'adset', ?, ?, ?, 'pixel', ?)""",
                (platform, adset_id, adset_id, campaign_id, now),
            )
        ...
        if ad_id:
            conn.execute(
                """INSERT OR REPLACE INTO ad_names
                   (platform, entity_type, entity_id, name, parent_id, source, updated_at)
                   VALUES (?, 'ad', ?, ?, ?, 'pixel', ?)""",
                (platform, ad_id, ad_id, adset_id, now),
            )
...
async def track_event(request: Request):
    ...
    # Auto-learn campaign → adset → ad hierarchy from this pixel hit
    if platform:
        _learn_ad_hierarchy(
            db_path,
            platform=platform,
...
async def track_conversion(request: Request):
    ...
    # Auto-learn campaign → adset → ad hierarchy from this conversion hit
    if platform:
        _learn_ad_hierarchy(
            db_path,
            platform=platform,
